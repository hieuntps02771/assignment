...
$sql = "SELECT id, listcatid, publtime, exptime, title, alias, hometext, homeimgthumb, homeimgfile FROM `" . NV_PREFIXLANG . "_" . $module_data . "_rows` WHERE `status`= 1 AND `publtime` < " . NV_CURRENTTIME . " AND (`exptime`=0 OR `exptime`>" . NV_CURRENTTIME . ") ORDER BY `publtime` DESC LIMIT 0 , 6";
...
while ( $row = $db->sql_fetchrow( $result ) )
{
    ...
    {
        $array_img = explode( "|", $row ['homeimgfile'] );
    }
    else
    {
        $array_img = array( 
    ...
    elseif ( $row ['homeimgfile'] != "" and file_exists( NV_BASE_SITEURL . NV_UPLOADS_DIR . '/' . $module_name . '/' . $row ['homeimgfile'] ) )
    {
